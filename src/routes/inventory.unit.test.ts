...
describe("inventory routes (unit)", () => {
  ...
  it("swaps stacks when merge is not possible but swapping is allowed", async () => {
    ...
    const tempWhere = vi.fn().mockResolvedValue([{ id: fromStack.id }]);
    const tempSet = vi.fn().mockReturnValue({ where: tempWhere });

    database.update
      .mockReturnValueOnce({ set: tempSet })
    ...
    expect([body.a.slot, body.b.slot].sort()).toEqual([0, 2]);
